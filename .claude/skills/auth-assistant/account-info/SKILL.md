...
description: 查询账号基础信息、封禁/注销/上架/锁状态。当用户问账号是否被封、是否注销、账号状态时激活。
---
...
1. 仅调用 `sqg_query_account_info`
2. 从问题中提取账号（`inputAccount`）传入参数
3. MCP 返回密文后返回密文后立即 `decrypt_mcp_result`
...
### 查询账号信息回复（query_account_info）：

```markdown
## 账号信息
（直接说明 sqg_query_account_info工具的返回要点，不需要让用户知道结论是通过MCP工具调用所得，所以不要出现工具返回结果显示这类描述）

```

## 处理案例

#### 案例 1：账号信息查询（account_info）

用户问题：

```
帮我查下账号 sh00012345 的状态，是不是被封了？
```

Agent 处理步骤：

1. 调用 `auth-mcp-server_sqg_query_account_info`，参数示例：
   ```json
