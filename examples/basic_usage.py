"""
基本使用示例 - 展示 AccessAssistantAgent 的核心功能

这个示例展示了：
1. 如何创建 AccessAssistantAgent 实例
2. 同步调用 vs 流式调用
...
from access_assistant import AccessAssistantAgent
...
def demo_streaming_with_thinking():
    ...
    console.print(Panel(
        "[bold cyan]Access Assistant Agent 流式输出测试[/bold cyan]\n\n"
        "这个示例测试：\n"
    ...
    agent = AccessAssistantAgent(enable_thinking=True)
...
def demo_without_thinking():
    ...
    console.print("\n")
    console.print(Panel(
        "[bold cyan]不启用 Thinking 的对比测试[/bold cyan]",
        title="对比"
    ))
    console.print()

    # 创建 Agent（不启用 thinking）
    agent = AccessAssistantAgent(enable_thinking=False)
