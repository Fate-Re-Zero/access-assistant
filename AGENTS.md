...
- `src/access_assistant/` houses the core package (agent, CLI, tools, skill loader, and `stream/` formatting/tracking helpers).
...
- `pyproject.toml` defines dependencies and the `access-assistant` CLI entry point; `uv.lock` pins versions.

## Build, Test, and Development Commands
- `uv sync`: install dependencies into the local environment.
- `uv run access-assistant --interactive`: run the interactive CLI demo.
- `uv run access-assistant "列出当前目录"`: run a single prompt.
- `uv run access-assistant --list-skills`: verify Skills discovery.
...
- Keep CLI output and stream formatting behavior consistent with existing utilities in `src/access_assistant/stream/`.
